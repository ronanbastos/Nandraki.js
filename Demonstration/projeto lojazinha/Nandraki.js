    ...
    gravity: function (obj = { up: 0, velocidade: 0, gravidade: 0.05, id: "player_id" }, target_id = [], force_jump = 5, bonce_bool = false, bounce_value = 0.8) {
        ...
        function myFunction(item) {
            ...
            if (colidir == true) {
                ...
                }else if(game.get_top(obj.id)<game.get_ty(item) && game.get_left(obj.id)<game.get_tx(item)  ){
                    pulo -= 1;     
                    obj.up = obj.up-obj.velocidade
                    obj.velocidade += (obj.velocidade * 50);
                    x-=20+game.objw(item)/game.get_tx(obj.id) 
                    obj.dir-=20+game.objw(item)/game.get_tx(obj.id) 
