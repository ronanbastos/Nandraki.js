...
class ThreeCore {
    ...
    updateHierarchy() {
        ...
        this.scene.children.forEach(obj => {
            ...
            if (obj.name !== "EditorCamera" && !(obj instanceof TransformControls)) {
     ...
     * @returns {object} A JSON-compatible object representing the scene.
     */
    serializeScene() {
        // We will add custom data (like scripts and tags) to each object's userData
        // before serializing. These need to be in a JSON-compatible format.
        ...
        // Use Three.js's built-in .toJSON() method.
        // It's important to understand that scene.toJSON() will serialize the entire scene graph,
        // including geometries, materials, textures, and the object hierarchy.
        // By filtering the objects we want to include, we can control what gets saved.
        // However, a simpler approach that is less prone to error is to serialize the entire scene
        // and then filter the resulting JSON object, but that can be less efficient.
        // The most direct way is to create a temporary scene, but this can be complex with object references.
        // A good compromise is to serialize the objects individually and collect them.
        const sceneJSON = this.scene.toJSON();

        // After serializing, we must revert any changes made for serialization (like Set to Array).
        // This ensures the live scene objects remain in their original state.
        ...
        // This is a reliable way to ensure they are not in the saved file.
     ...
     * @param {object} sceneData - The JSON-compatible object representing the scene.
     */
    async deserializeScene(sceneData) {
        ...
        try {
            // The `parse` method reconstructs the scene from the JSON data.
            // It automatically creates geometries, materials, and objects.
            ...
            alert("Error: The scene file could not be parsed. It might be corrupted or in an old format.");
            return;
        }
